...
In practice, our observations will never be drawn from a unit Fréchet
...
coefficient (or smoothing parameter) equal to 0.5.
...
It is sometimes useful to fit several models to our data and then
compare the models together. To this aim, the well-known Akaike
information criterion (AIC) is often used. Because we work under
