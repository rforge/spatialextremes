...
$K_{\mbox{smooth}}$ is the modified Bessel function of the third kind
with order \textsl{smooth}.
...
be the Whittle-Matérn covariance function with scale and smooth
...
this estimator is tagged \textsl{grad} as it uses the gradient of the
...
tagged \textsl{score}, is given by the sample variance of each
...
likelihood estimator $\hat{\theta}$ satisfies:
\begin{equation*}
...
observations have unit Fréchet margins.

Let us recall that the log pairwise likelihood is defined by:
\begin{equation*}
  \ell_p(\mathbf{z}, \Sigma^{-1}) = \sum_{k = 1}^{n_{obs}}
  \sum_{i=1}^{n_{site}-1} \sum_{j=i+1}^{n_{site}} \log f(z_k^{(i)}, z_k^{(j)})
\end{equation*}
...
\begin{equation*}
  \nabla \ell_p(\theta) = \sum_{i=1}^{n_{site}-1}
...
\begin{equation*}
  \nabla \log f(z_k^{(i)}, z_k^{(j)}) = \frac{\partial}{\partial a}
  \log f(z_k^{(i)}, z_k^{(j)}) \left( \frac{\partial a}{\partial
...
supposed to be ordinary GEV. Consequently, we need a response surface
...
\begin{eqnarray}
  ...
  \mathbf{\mu} &=& X_\mu \beta_\mu\\
  \mathbf{\sigma} &=& X_\sigma \beta_\sigma\\
  \mathbf{\xi} &=& X_\xi \beta_\xi
\end{eqnarray}
where $\mathbf{\mu} = (\mu_1, \ldots, \mu_{n_{site}})$,
$\mathbf{\sigma} = (\sigma_1, \ldots, \sigma_{n_{site}})$ and
$\mathbf{\xi} = (\xi_1, \ldots, \xi_{n_{site}})$ are the vector for
the location, scale and shape GEV parameters for all the sites within
the region study, $X_\mu$, $X_\sigma$ and $X_\xi$ are the design
matrices for each GEV parameters and $\beta_\mu$, $\beta_\sigma$ and
$\beta_\xi$ are the regression coefficients to be estimated.

Consequently, from one ordinary GEV observation $\mathbf{x}$, one can
transform it to unit Fréchet margins using the following
transformation:
\begin{equation}
  \label{eq:gev2frech}
  \mathbf{z}^{(i)}=\left\{1+\frac{X^{(i)}_\xi
      \beta_\xi(\mathbf{x}^{(i)}-X^{(i)}_\mu
...
$\mathbf{z}^{(i)}$ denotes the $i$-th element of the vector
$\mathbf{z}$.


Consequently, it is easy to see that:
\begin{eqnarray*}
  \frac{\partial \mathbf{z}^{(i)}}{\partial \beta_\mu} &=&
  -\frac{(\mathbf{z}^{(i)})^{1 - X^{(i)}_\xi \beta_\xi} X^{(i)}_\mu}{
    X^{(i)}_\sigma \beta_\sigma}\\
  &=& -\frac{(\mathbf{z}^{(i)})^{1-\xi_i}}{\sigma_i}\cdot
  X_\mu^{(i)}\\
  \frac{\partial \mathbf{z}^{(i)}}{\partial \beta_\sigma} &=&
  -\frac{(\mathbf{z}^{(i)})^{1 - X^{(i)}_\xi \beta_\xi}
    \left(\mathbf{y}^{(i)} - X^{(i)}_\mu
      \beta_\mu\right)}{X^{(i)}_\sigma \beta^2_\sigma}\\
  &=& - \frac{(\mathbf{z}^{(i)})^{1-\xi_i} \left(\mathbf{y}^{(i)} -
      \mu_i \right)}{\sigma_i} \cdot \frac{1}{\beta_\sigma}\\ 
  \frac{\partial \mathbf{z}^{(i)}}{\partial \beta_\xi} &=& -
  \frac{\mathbf{z}^{(i)} \log \mathbf{z}^{(i)}}{\beta_\xi} +
  \frac{\mathbf{z}^{(i)} \left(\mathbf{y}^{(i)} - X^{(i)}_\mu
      \beta_\mu\right)}{\beta_\xi X^{(i)}_\sigma \beta_\sigma
    (\mathbf{z}^{(i)})^{X^{(i)}_\xi \beta_\xi}}\\
  &=& \left[ (\mathbf{z}^{(i)})^{1 - \xi_i}
    \frac{\left(\mathbf{y}^{(i)} - \mu_i\right)}{\sigma_i} -
    \mathbf{z}^{(i)} \log \mathbf{z}^{(i)} \right] \cdot
  \frac{1}{\beta_\xi}
...
need to compute the partial derivatives of $A$, $B$, $C$ and $D$
with respect to $\beta_\mu$, $\beta_\sigma$ and $\beta_\xi$. However,
for clarity purpose, we compute first the following derivatives.
...
Now we're ready for the partial derivatives of $A$, $B$, $C$ and $D$
w.r.t. $\beta_\mu$, $\beta_\sigma$ and $\beta_\xi$. For shortness, we
do it in one step with the notation $\beta = (\beta_\mu, \beta_\sigma,
\beta_\xi)$.
\begin{eqnarray*}
  ...
  \nabla_\beta z_1\\
\end{eqnarray*}
where $\nabla_\beta z_1$ is the gradient of $z_1$ w.r.t. $\beta$ and
the partial derivatives of $A$, $B$, $C$ and $D$ w.r.t. $z_1$ are
...
\begin{eqnarray*}
  ...
  dB_{z_1} &=& \frac{\partial B}{\partial z_1} = \frac{c_1
    ...
    z_2}\\
  dC_{z_1} &=& \frac{\partial C}{\partial z_1} = - \frac{(c_2 + 2a)
    \varphi(c_1)}{a^2 z_1^3} - \frac{\Phi(c_1)}{z_1^3} +
  \frac{(a+c_2)\varphi(c_2)}{a^2z_1^2z_2}\\
...
\bibliography{/home/ribatet/Documents/Biblio/biblio_ribatet}
