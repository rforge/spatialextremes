...
extremal coefficient function using the \verb|extcoeff| function.
<<label=extcoeff>>=
extcoeff(fitted)
@ 
...
For illustration purpose, let compare the fitted covariance function
to the theoretical one on a new artificial example.
...
plot(covariance(fitted), from = 0, to = 200, ylim = c(0,1))
plot(covariance(scale = 30, smooth = 1.2, cov.mod = "cauchy"),
col = 3, add = TRUE, from = 0, to = 200)
...
One can have a contour plot with different color palettes for the
evolution of the GEV parameters in $\mathbb{R}^d$ by invoking the
following code:
...
We have to be aware that the bivariate density changed when we do not
suppose unit Fréchet margins anymore. For instance, the bivariate
density evaluated at two observation $y_1$ and $y_2$ with ordinary GEV
margins is given by:
\begin{equation}
...
\begin{eqnarray*}
  ...
  dC_{z_1} &=& \frac{\partial C}{\partial z_1} = - \frac{(c_2 + 2a)
    \varphi(c_1)}{a^2 z_1^3} - \frac{2\Phi(c_1)}{z_1^3} +
...
\begin{eqnarray*}
  ...
  dB_{z_2} &=& \frac{\partial B}{\partial z_2} = - \frac{(c_1 + 2a)
    \varphi(c_2)}{a^2 z_2^3} - \frac{2\Phi(c_2)}{z_2^3} +
  \frac{(a+c_1)\varphi(c_2)}{a^2z_1z_2^2}\\
  dC_{z_2} &=& \frac{\partial C}{\partial z_2} = \frac{c_1
    \varphi(c_2)}{a^2 z_1 z_2^2} + \frac{c_2 \varphi(c_1)}{a^2 z_1^2
    z_2}\\
  dD_{z_2} &=& \frac{\partial D}{\partial z_2} = -
  \frac{\left[1+c_2(a+c_1) \right]\varphi(c_1)}{a^3 z_1^2 z_2^2} + 
  \frac{\left[1 - c_1(a+c_1)\right] \varphi(c_2)}{a^3z_1z_2^3}
\end{eqnarray*}

For the Jacobian part $E$, we have:
\begin{eqnarray*}
  dE_\mu &=& \frac{\partial E}{\partial \beta_\mu} = -
  \frac{1-\xi_1}{\sigma_1 z_1^{-\xi_1}} \cdot X^{(1)}_\mu -
  \frac{1-\xi_2}{\sigma_2 z_2^{-\xi_2}} \cdot X^{(2)}_\mu\\
  dE_\sigma &=& \frac{\partial E}{\partial \beta_\sigma} =  - \left(
    \frac{(y_1 - \mu_1)(1 - \xi_1)}{\sigma_1z_1^{\xi_1}} +
    \frac{(y_2-\mu_2)(1 - \xi_2)}{\sigma_2 z_2^{\xi_2}} + 2
    z_1^{2\xi_1-2} z_2^{2\xi_2-2}\right) \cdot
  \frac{1}{\beta_\sigma}\\
...
where
\begin{equation*}
  \frac{\partial E}{\partial \beta} = \left(\frac{\partial E}{\partial
      \beta_\mu}, \frac{\partial E}{\partial \beta_\sigma},
    \frac{\partial E}{\partial \beta_\xi} \right)^T  
\end{equation*}
