...
\begin{titlepage}
  ...
  \begin{center}
    ...
    \vspace{1em}
    \Large Mathieu Ribatet$^\dag$ \and Simone Padoan$^\ast$\\
    \vspace{1em}
...
Now we can see that only the scale parameter was estimated and
...
fitmaxstab(ms0, locations, cov.mod = "cauchy", scale = 3, fit.marge = FALSE)
@ 
...
covariance(scale = 3, smooth = 1.2, cov.mod = "cauchy", col = 3, add = TRUE)
...
rbind(fitted = covariance(fitted, dist = seq(0,10, 3))$cov.val,
      theo = covariance(scale = 3, smooth = 1.2, cov.mod = "cauchy", dist = seq(0,10, 3))$cov.val)
@ 
...
\bibliography{/home/ribatet/Biblio/biblio_ribatet}
