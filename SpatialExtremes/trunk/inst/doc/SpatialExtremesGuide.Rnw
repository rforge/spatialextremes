...
\begin{tabular}{ll}
  ...
  \textbf{Cauchy} & $\rho(h) = \left[1 + \left(\frac{h}{range}
...
where $h$ is the distance between locations \#1 and \#2, $range$ and
$smooth$ are the range and smooth parameter of the covariance
...
Whittle-Matérn covariance function with range and smooth parameters
equal to 3 and 1.2 respectively. The locations are distributed
uniformly on the square $[0,10]^2$.

To fit a max-stable process assuming unit Fréchet margins, all we have
to do is to invoke:
<<>>=
...
Now we can see that only the range parameter was estimated and
standard errors as well as the TIC are available.
...
\ell_p(\psi;\mathbf{Y})]$ (the Jacobian matrix), where the
...
<<>>=
model1 <- fitmaxstab(ms0, locations, cov.mod = "gauss", fit.marge = FALSE)
model2 <- fitmaxstab(ms0, locations, cov.mod = "powexp", fit.marge = FALSE)
TIC(model1, model2)
@ 

Fortunately, with this simulated application, we clearly see that our
\verb|model1| is the best model.
...
the extremal coefficient as the (euclidean) distance increase. The
first one is to use an empirical estimation of the extremal
coefficient; while the second use a parametric estimation using the
Smith's and Schlather's models.

\citet{Schlather2003} introduce a methodology to estimate non
parametrically the extremal coefficient. The \verb|extcoeff.emp|
...
the following lines and Figure plots the resulting output.
<<label=extcoeff.emp>>=
n.site <- 30
locations <- matrix(runif(2*n.site, 0, 10), ncol = 2)
colnames(locations) <- c("lon", "lat")
...
param=c(0,1,0,3, 1.2), maxstable="extr", n = 60)
ms1 <- t(ms0)
...
covariance(sill = 1, range = 3, smooth = 1.2, cov.mod = "cauchy", col = 3, add = TRUE)
...
rbind(fitted = covariance(fitted, dist = seq(0,10, 3))$cov.val,
      theo = covariance(sill = 1, range = 3, smooth = 1.2, cov.mod = "cauchy", dist = seq(0,10, 3))$cov.val)
@ 
...
ms0 <- MaxStableRF(locations[,1], locations[,2], grid=FALSE, model="wh",
                   param=c(0,1,0,3, .5), maxstable="extr", n = 40)
ms0 <- t(ms0)
ms1 <- ms0
...
schlather <- fitmaxstab(ms1, locations, "whitmat", loc.form, scale.form,
                        shape.form, control = list(ndeps = rep(10^-4, 30)))
@ 
