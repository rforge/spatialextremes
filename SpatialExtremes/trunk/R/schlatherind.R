...
schlatherindfull <- function(data, coord, start, cov.mod = "whitmat", ...,
  ...
  if (fit.marge){
    ...
    body(nplk) <- parse(text = paste("-.C('schlatherindfull', as.integer(cov.mod.num), as.double(data), as.double(dist), as.integer(n.site), as.integer(n.obs),",
