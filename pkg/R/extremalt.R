...
extremaltfull <- function(data, coord, start, cov.mod = "whitmat", ...,
    ...
    body(nplk) <- parse(text = paste("-.C('extremaltfull', as.integer(cov.mod.num), as.double(data), as.double(dist), as.integer(n.site), as.integer(n.obs), as.integer(dist.dim), as.integer(weighted), as.double(weights),",
                        ...
                        "as.double(sill), as.double(range), as.double(smooth), as.double(smooth2), as.double(DoF), fit.marge, dns = double(1), PACKAGE = 'SpatialExtremes')$dns"))
    ...
    if (missing(start)) {
        ...
        if (length(fixed.param) > 0){
            ...
            cov.start <- fitcovariance(data, coord, paste("t", cov.mod, sep=""),
                                       marge = "emp")$param
...
extremaltform <- function(data, coord, cov.mod, loc.form, scale.form, shape.form,
    ...
    body(nplk) <- parse(text = paste("-.C('extremaltdsgnmat', as.integer(cov.mod.num),
...
as.double(DoF), dns = double(1), PACKAGE = 'SpatialExtremes')$dns"))
    ...
    if (missing(start)) {
        ...
        start <- .start.extremalt(data, coord, covariables, cov.mod, loc.form,
