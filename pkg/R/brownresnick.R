...
brownresnickfull <- function(data, coord, start, fit.marge = FALSE, ...,
    ...
    if (missing(start)) {
        ...
        if (length(fixed.param) > 0){
            args <- c(list(data = data, coord = coord, cov.mod = "brown", marge = "emp"),
                      fixed.param)
            ...
            cov.start <- fitcovariance(data, coord, "brown", marge = "emp")$param
    ...
    std.err <- .brownresnickstderr(param, data, dist, as.double(0), as.double(0), as.double(0),
                                   as.double(0), as.double(0), as.double(0), rep(FALSE, 3),
                                   fit.marge = fit.marge,
...
##parameters
brownresnickform <- function(data, coord, loc.form, scale.form, shape.form, start,
                             fit.marge = TRUE, marg.cov = NULL, ..., warn = TRUE,
    ...
    param <- c("range", "smooth", loc.names, scale.names, shape.names, temp.names.loc,
