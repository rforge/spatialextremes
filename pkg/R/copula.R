...
as.double(dist), as.double(data), as.integer(n.site), as.integer(n.obs), as.integer(dist.dim),
...
as.double(smooth2), dns = double(1), PACKAGE = 'SpatialExtremes')$dns"))
  ...
  if (missing(start)){
    ...
    start <- as.list(c(sill = 1, range = 0.25 * max(dist), smooth = 1, locCoeff, scaleCoeff,
