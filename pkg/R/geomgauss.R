...
geomgaussfull <- function(data, coord, start, cov.mod = "whitmat", ...,
  ...
  if (fit.marge){
    ...
    body(nplk) <- parse(text = paste("-.C('geomgaussfull', as.integer(cov.mod.num), as.double(data), as.double(dist), as.integer(n.site), as.integer(n.obs),",
