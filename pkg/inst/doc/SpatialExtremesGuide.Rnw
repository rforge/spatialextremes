...
\begin{tabular}{lllll}
  ...
  K_{\nu}\left(\frac{h}{c_2} \right)$, & $0 \leq c_1 \leq 1$, & $c_2
  >0$, & $\nu > 0$\\  
...
@ 
  \caption{Impact of the number of knots in the fitted
    ...
    panel: $q=100$. The small vertical lines corresponds to the
...
model~\eqref{eq:decompBasis2} with $q=2, 10$ and $100$. Clearly, the
...
overfits. Consequently, there is a pressing need for a kind of
...
curve. It can be shown that problem~\eqref{eq:constraintLeastSquare}
has the solution
\begin{equation}
...
\begin{equation}
  ...
  CV(\lambda) = \sum_{i=1}^n \{y_i - \hat{f}_{-i}(x_i;\lambda)\}^2 =
  \sum_{i=1}^n \left(\frac{y_i - \hat{y}_i}{1 -
...
\textbf{Write something about GCV}.
...
CV or GCV lead to indistinguishable curves. The code to generate
