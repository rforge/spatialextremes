...
Smith's model\footnote{Another form of Smith's model that uses a
  Student distribution instead of the Normal one. However, it is not
  currently implemented.} \citep{Smith1991} is given by:
\begin{equation}
  \label{eq:smith}
...
handle 2 by 2 or 3 by 3 covariance matrices $\Sigma$.

\section{The Schlather Model}
\label{sec:schl-char}

Schlather's model \citep{Schlather2002} is given by:
\begin{equation}
  \label{eq:schlather}
...
the ``full'' likelihood. The log pairwise-likelihood is given
by
\begin{equation}
  \label{eq:lplik}
...
fit either the Smith or the Schlather models using least squares.
...
If now, we want to fit the model using the least squares:
<<eval=FALSE>>=
##Smith's model
...
the models using a criteria such as the well-known Akaike information
criterion (AIC. Because we work under miss-specification, AIC is not
