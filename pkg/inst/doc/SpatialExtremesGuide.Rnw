...
data <- rmaxstab(40, coord, "whitmat", sill = 1, range = 1, smooth = 2)

##Compute the lambda-madogram
lmadogram(data, coord, n.bins = 25, border = "grey", box = FALSE, rgl = FALSE)
@ 
  \end{center}
\end{titlepage}
...
par(mfrow=c(1,2))
image(x, x, log(data0[,,1]), col = terrain.colors(40))
image(x, x, log(data1[,,1]), col = terrain.colors(40))
@ 
...
covariance(sill = 1, range = 1, smooth = 2, cov.mod = "bessel",
           xlim = c(0, 20), ylim = c(-0.3, 1))
covariance(sill = 1, range = 1, smooth = 1, cov.mod = "bessel",
           add = TRUE, col = 2, xlim = c(0, 20))
covariance(sill = 1, range = 1, smooth = .5, cov.mod = "bessel",
           add = TRUE, col = 3, xlim = c(0, 20))
covariance(sill = 1, range = 1, smooth = 0.25, cov.mod = "bessel",
...
par(mfrow=c(1,2))
image(x, y, log(ms0), col = terrain.colors(30))
image(x, y, log(ms1), col = terrain.colors(30))
@ 
...
\begin{proof}
  ...
  \begin{equation*}
    \mathbb{E}[Y Y^T] = \mathbb{E}[D^{1/2} X X^T (D^{1/2})^T] =
    \mathbb{E}[D^{1/2} I_n (D^{1/2})^T] = D^{1/2} (D^{1/2})^T = D
  \end{equation*}
...
par(mfrow=c(1,3))
image(x, y, data1[,,1], col = terrain.colors(30))
image(x, y, data2[,,1], col = terrain.colors(30))
image(x, y, data3[,,1], col = terrain.colors(30))
@ 
...
<<echo=FALSE,label=rmaxstabSmith>>=
x <- y <- seq(0, 10, length = 100)
coord <- cbind(x, y)
set.seed(8)
...
par(mfrow = c(1,2))
image(x, y, log(M0[,,1]), col = terrain.colors(30))
image(x, y, log(M1[,,1]), col = terrain.colors(30))
@ 
...
data1 <- rmaxstab(100, coord, "whitmat", sill = 1, range = 1, smooth = 1)
@ 
while for locations located on a grid, users should invoke
<<eval=FALSE>>=
x <- seq(0, 10, length = 100)
coord <- cbind(x, x)
data2 <- rmaxstab(1, coord, "powexp", sill = 1, range = 1, smooth = 2,
                  grid = TRUE) 
@ 
...
n.obs <- 100
coord <- matrix(runif(2 * n.site, 0, 10), ncol = 2)
data <- rmaxstab(n.obs, coord, "whitmat", sill = 1, range = 1,
                 smooth = 1)
...
n.obs <- 100
coord <- matrix(runif(2 * n.site, 0, 10), ncol = 2)
data <- rmaxstab(n.obs, coord, "whitmat", sill = 1, range = 1, smooth = 1)
...
madogram(data, coord, which = "ext", ylim = c(1, 2))
...
fmadogram(data, coord, which = "ext", ylim = c(1, 2.4))
...
n.obs <- 100
coord <- matrix(runif(2 * n.site, 0, 10), ncol = 2)
data1 <- rmaxstab(n.obs, coord, "powexp", sill = 1, range = 1, smooth = 1)
data2 <- rmaxstab(n.obs, coord, "cauchy", sill = 1, range = 1, smooth = 1)

par(mfrow=c(1,2), pty = "s")
lmadogram(data1, coord, n.bins = 60, rgl = FALSE)
lmadogram(data2, coord, n.bins = 60, rgl = FALSE)
@ 
...
set.seed(12)
coord <- matrix(runif(2*n.site, 0, 10), ncol = 2)
data <- rmaxstab(n.obs, coord, "whitmat", sill = .2, range = 3, smooth = 1.2)

fitmaxstab(data, coord, cov.mod = "whitmat", std.err.type = "none")
@ 
...
fitmaxstab(data, coord, cov.mod = "whitmat", sill = 1)
...
$J(\psi)$ corresponds to the variance of the pairwise score
equation\index{score equation} $\ell_p(\psi;\mathbf{Y}) =
0$. Consequently, a second estimator, tagged \verb|score|, is given by
the sample variance of each contribution to the pairwise score
function. These two estimators are only accessible if independent
replications of $\mathbf{Y}$ are available\footnote{which will mostly
  be the case for spatial extremes.}.

These two types of standard errors are available by invoking the
following two lines:
<<eval=FALSE>>=
fitmaxstab(data, coord, cov.mod = "gauss", std.err.type = "score")
fitmaxstab(data, coord, cov.mod = "gauss", std.err.type = "grad")
@ 
...
n.site <- 40
coord <- matrix(runif(2*n.site, 0, 10), ncol = 2)
data <- rmaxstab(n.obs, coord, "cauchy", sill = 0.8, range = 3, smooth = 1.2)
