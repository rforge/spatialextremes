...
void schlathergrad(int *covmod, double *data, double *dist, int *nSite,
  ...
  for (k=0;k<*nObs;k++){
	...
	case 4:
	  //i.e. Bessel
	  grad[*nObs + k] += *sill * R_pow(2 * *range / dist[currentPair], *smooth) *
	...
	case 5:
	  ...
	  grad[*nObs + k] += rho[currentPair] * *smooth / *range /
	    ...
	    jacCommonRho;
	  grad[2 * *nObs + k] -= log1p(R_pow(dist[currentPair] / *range, *smooth2)) *
	    rho[currentPair] / *smooth2 *jacCommonRho;
	  grad[3 * *nObs + k] += (log1p(R_pow(dist[currentPair] / *range, *smooth2)) /
  ...
  if (*fitmarge){
	...
	for (j=i+1;j<*nSite;j++){
	  ...
	  for (l=0;l<*nloccoeff;l++){
	    ...
	    grad[(nCorPar + l) * *nObs + k] += (dAz1 * dz1loc + dAz2 * dz2loc) + 
	  ...
	  for (l=0;l<*nscalecoeff;l++){
	    ...
	    grad[(nCorPar + *nloccoeff + l) * *nObs + k] += (dAz1 * dz1scale + dAz2 * dz2scale) +
...
void schlatherindgrad(int *covmod, double *data, double *dist, int *nSite,
  ...
  for (k=0;k<*nObs;k++){
	...
	case 4:
	  //i.e. Bessel
	  grad[2 * *nObs + k] += *sill * R_pow(2 * *range / dist[currentPair], *smooth) *
	    dist[currentPair] / (*range * *range) * gammafn(*smooth + 1) *
	    bessel_j(dist[currentPair] / *range, *smooth + 1) * jacCommonRho;
	  grad[3 * *nObs + k] = R_NaReal;
	  break;
	  case 5:
	  //i.e. Generalized Cauchy
	  grad[2 * *nObs + k] += rho[currentPair] * *smooth / *range /
	    (1 + R_pow(*range / dist[currentPair], *smooth2)) *
	    jacCommonRho;
	  grad[3 * *nObs + k] -= log1p(R_pow(dist[currentPair] / *range, *smooth2)) *
	    rho[currentPair] / *smooth2 *jacCommonRho;
	  grad[4 * *nObs + k] += (log1p(R_pow(dist[currentPair] / *range, *smooth2)) /
				  *smooth2 - log(dist[currentPair] / *range) /
				  (1 + R_pow(*range / dist[currentPair], *smooth2))) *
	    *smooth / *smooth2 * rho[currentPair] * jacCommonRho;
	  //The caugen has 2 smooth paramaters so add + 1 to nCorPar.
...
void spatgevgrad(double *data, int *nSite, int *nObs, double *locdsgnmat,
  ...
  for (i=0;i<*nObs;i++){
	...
	grad[(*nloccoeff + *nscalecoeff + k) * *nObs + i] += 
	  (log1p(shapes[j] * (data[j * *nObs + i] - locs[j]) / scales[j]) / shapes[j] -
	   (1 + shapes[j]) * (data[j * *nObs + i] - locs[j]) / scales[j] /
	   (1 + shapes[j] * (data[j * *nObs + i] - locs[j]) / scales[j]) -
...
void geomgaussgrad(int *covmod, double *data, double *dist, int *nSite,
  ...
  for (k=0;k<*nObs;k++){
	...
	case 4:
	  //i.e. Bessel
	  grad[2 * *nObs + k] -= *sigma2 * *sill * R_pow(2 * *range / dist[currentPair], *smooth) *
	    dist[currentPair] / (*range * *range * mahalDist[currentPair]) * gammafn(*smooth + 1) *
	    bessel_j(dist[currentPair] / *range, *smooth + 1) * jacCommon;
	  grad[3 * *nObs + k] = R_NaReal;
	  break;
	case 5:
	  //i.e. Generalized Cauchy
	  grad[2 * *nObs + k] -= *sigma2 * rho * *smooth / 
	    (*range * (1 + R_pow(*range / dist[currentPair], *smooth2)) * mahalDist[currentPair]) *
	    jacCommon;
	  grad[3 * *nObs + k] += *sigma2 * log1p(R_pow(dist[currentPair] / *range, *smooth2)) *
	    rho / (*smooth2 * mahalDist[currentPair])* jacCommon;
	  grad[4 * *nObs + k] -= *sigma2 * (log1p(R_pow(dist[currentPair] / *range, *smooth2)) /
				  *smooth2 - log(dist[currentPair] / *range) /
				  (1 + R_pow(*range / dist[currentPair], *smooth2))) *
...
void brownresnickgrad(double *data, double *dist, int *nSite, int *nObs,
  ...
  for (k=0;k<*nObs;k++){
	...
	grad[k] -= 0.5 * *smooth * mahalDist[currentPair] / *range * jacCommon;
	grad[*nObs + k] += 0.5 * log(dist[currentPair] / *range) * mahalDist[currentPair] *
	  jacCommon;
