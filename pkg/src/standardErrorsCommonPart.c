...
void marginalPartExtremalt(int *start, int *nObs, int *nSite, double *data, double *frech,
  ...
  for (i=0;i<(*nSite-1);i++){
	...
	for (l=0;l<*nloccoeff;l++){
	  ...
	  hess[((idx + l) * *nObs + k) * nPairs + currentPair] = (dAz1 * dz1loc + dAz2 * dz2loc) +
	    ((dDz1 * dz1loc + dDz2 * dz2loc) + (dBz1 * dz1loc + dBz2 * dz2loc) * C + B *
	     (dCz1 * dz1loc + dCz2 * dz2loc)) * iBCplusD + dE;
	  grad[(idx + l) * *nObs + k] += hess[((idx + l) * *nObs + k) * nPairs + currentPair];
	}
	...
	for (l=0;l<*nscalecoeff;l++){
	  ...
	  hess[((idx + l) * *nObs + k) * nPairs + currentPair] = (dAz1 * dz1scale + dAz2 * dz2scale) +
	    ((dDz1 * dz1scale + dDz2 * dz2scale) + (dBz1 * dz1scale + dBz2 * dz2scale) * C + B *
	     (dCz1 * dz1scale + dCz2 * dz2scale)) * iBCplusD + dE;
	  grad[(idx + l) * *nObs + k] += hess[((idx + l) * *nObs + k) * nPairs + currentPair];
	}
	...
	for (l=0;l<*nshapecoeff;l++){
	  ...
	  hess[((idx + l) * *nObs + k) * nPairs + currentPair] = (dAz1 * dz1shape + dAz2 * dz2shape) +
	    ((dDz1 * dz1shape + dDz2 * dz2shape) + (dBz1 * dz1shape + dBz2 * dz2shape) * C +
	     (dCz1 * dz1shape + dCz2 * dz2shape) * B) * iBCplusD + dE;
	  grad[(idx + l) * *nObs + k] +=  hess[((idx + l) * *nObs + k) * nPairs + currentPair];
	}
